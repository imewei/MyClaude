...
DIAGNOSE_SCRIPT = (
    PLUGIN_ROOT / "skills/nlsq-core-mastery/scripts/diagnose_optimization.py"
)
COMPARE_SCRIPT = (
    PLUGIN_ROOT / "skills/training-diagnostics/scripts/compare_training_runs.py"
)
...
class TestDiagnoseOptimization(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_jnp.abs.return_value = (
            MagicMock()
        )  # abs returns a mock (array), max of that is float
        self.mock_jnp.isnan.return_value = False
        self.mock_jnp.isinf.return_value = False

        with patch.dict(
            sys.modules, {"jax": self.mock_jax, "jax.numpy": self.mock_jnp}
        ):
    ...
    def test_analyze_convergence_converged(self):
        ...
        result.fun = self.mock_jnp.array(
            [0.0, 0.0]
        )  # mock_jnp.array returns a mock but we need it to work with mean/std
...
class TestCompareTrainingRuns(unittest.TestCase):
    ...
    def test_print_comparison_report(self):
        ...
        with patch("builtins.print") as mock_print:
            self.module.print_comparison_report(
                configs, metrics, primary_metric="val_loss"
            )

            # Check for header
            self.assertTrue(
                any("COMPARISON REPORT" in str(c) for c in mock_print.call_args_list)
            )
