...
class TestSupplementaryDocs(unittest.TestCase):
    ...
    def test_glossary_has_minimum_20_terms(self):
        ...
        self.assertTrue(
            glossary_path.exists(), f"Glossary file not found at {glossary_path}"
        )
        ...
        self.assertGreaterEqual(
            term_count, 20, f"Glossary contains only {term_count} terms (20 required)"
        )
    ...
    def test_quick_start_guides_exist(self):
        ...
        self.assertTrue(
            guides_dir.exists(), f"Guides directory not found at {guides_dir}"
        )
        ...
        self.assertGreaterEqual(
            len(existing_guides),
            5,
            f"Only {len(existing_guides)} guides exist (5 required)",
        )
    ...
    def test_glossary_term_links_work(self):
        ...
        self.assertIn(
            ".. glossary::", content, "Glossary missing '.. glossary::' directive"
        )

        # Check for proper indentation
        glossary_section = (
            content.split(".. glossary::")[1] if ".. glossary::" in content else ""
        )
        lines_after_directive = [
            line for line in glossary_section.split("\n")[1:] if line.strip()
        ]
        self.assertGreaterEqual(
            len(lines_after_directive),
            3,
            "Glossary appears empty or improperly formatted",
        )
    ...
    def test_guide_code_examples_render(self):
        ...
        self.assertGreaterEqual(
            code_blocks_found,
            3,
            f"Only {code_blocks_found} guides have code examples (expected at least 3)",
        )
    ...
    def test_changelog_follows_version_structure(self):
        ...
        self.assertTrue(
            changelog_path.exists(), f"Changelog file not found at {changelog_path}"
        )
    ...
    def test_guides_have_multi_plugin_workflows(self):
        ...
        for guide_path in guides:
            ...
            indicators_found = sum(
                1 for indicator in workflow_indicators if indicator in content
            )
            if indicators_found >= 2:
                guides_with_workflow += 1

        self.assertGreaterEqual(
            guides_with_workflow,
            3,
            f"Only {guides_with_workflow} guides have workflow content (expected at least 3)",
        )
    ...
    def test_guides_include_cross_references(self):
        ...
        self.assertGreaterEqual(
            guides_with_refs,
            3,
            f"Only {guides_with_refs} guides have cross-references (expected at least 3)",
        )
    ...
    def test_glossary_includes_required_terms(self):
        ...
        self.assertGreaterEqual(
            coverage,
            80,
            f"Glossary missing key terms ({coverage:.0f}% coverage). Missing: {', '.join(missing_terms[:5])}...",
        )
