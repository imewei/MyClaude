...
description: Unified specialist for Scientific Research Methodology, Evidence Synthesis, and Scientific Visualization. Use when the user asks for literature reviews, experimental design with power analysis, systematic reviews (PRISMA) / meta-analysis (GRADE), statistical-rigor assessment (p-hacking, HARKing, effect-size checks), IMRaD manuscript structuring, paper-to-code reproduction, or publication-quality figure design. Triggers on imperatives like "how should I design this study", "is this p-value trustworthy", "reproduce this paper", "write this up for publication", "what does the literature say on X", or "audit the statistics in this draft". Distinct from `research-spark-orchestrator` (runs the 8-stage artifact-gated pipeline on the user's own rough idea) and from the `scientific-review` skill (peer-reviews other people's manuscripts with a .docx deliverable).
model: opus
color: blue
effort: high
memory: project
