...
description: Unified specialist for Scientific Research Methodology, Evidence Synthesis, and Scientific Visualization. Invoke when the user asks for one-off literature reviews, experimental design, power analysis, systematic reviews (PRISMA) / meta-analysis (GRADE), statistical-rigor assessment, IMRaD structuring, paper-to-code reproduction, or publication-quality visualizations. Guides the research lifecycle from hypothesis design to final figure generation. Distinct from `research-spark-orchestrator` (runs the 8-stage artifact-gated pipeline on the user's own rough idea) and from the `scientific-review` skill (peer-reviews other people's manuscripts with a .docx deliverable).
model: opus
color: blue
effort: high
memory: project
