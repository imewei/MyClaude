...
description: Unified specialist for Scientific Research Methodology, Evidence Synthesis, and Scientific Visualization. Use when the user asks for literature reviews, experimental design with power analysis, systematic reviews (PRISMA) / meta-analysis (GRADE), statistical-rigor assessment (p-hacking, HARKing, effect-size checks), IMRaD manuscript structuring, methodology extraction for paper implementation, or publication-quality figure design. Triggers on imperatives like "how should I design this study", "is this p-value trustworthy", "write this up for publication", "what does the literature say on X", or "audit the statistics in this draft". For executable paper reproduction, route to /paper-implement or /replicate. Distinct from `research-spark-orchestrator` (runs the 8-stage artifact-gated pipeline on the user's own rough idea) and from the `scientific-review` skill (peer-reviews other people's manuscripts with a .docx deliverable).
model: opus
color: blue
effort: high
memory: project
maxTurns: 50
tools: Read, Grep, Glob, WebSearch, WebFetch, Bash, EnterPlanMode, ExitPlanMode
background: true
skills:
  - research-practice
---
