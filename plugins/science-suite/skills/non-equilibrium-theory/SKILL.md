...
Generate forward/reverse work samples using the `stochastic-dynamics` Langevin pattern with `jax.vmap` over N replicas. Accumulate `W = ∫ (∂H/∂λ) dλ` along a linear λ schedule.
...
dF     = result["Delta_f"]  * kT
dF_err = result["dDelta_f"] * kT
```

For multiple states, use `pymbar.MBAR(u_kn, N_k).compute_free_energy_differences()`. Production MD: use `alchemlyb` wrapper for LAMMPS/GROMACS/NAMD/AMBER/OpenMM output. See `advanced-simulations`.
...
- Langevin ensembles, SDE numerics → `stochastic-dynamics`
- Equilibrium stat-mech (ensembles, phase transitions, MC) → `statistical-physics`
- Rare-event samplers (WESTPA, OPS, milestoning) → `advanced-simulations`, `rare-events-sampling`
- Power-law tail fitting, SOC exponents → `extreme-value-statistics`
- Active-matter entropy production → `active-matter`
- Equation discovery from trajectories → `equation-discovery`, `bayesian-sindy-workflow`
