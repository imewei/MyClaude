...
def diagnose_gradient_pathology(
    ...
    if len(norms) > 0:
        ...
        if max_norm > 100:
            errors.append(
                f"🔥 EXPLODING GRADIENTS: Max norm = {max_norm:.2e} (threshold: 100)"
            )
            exploding_layers = [name for name, norm in norms.items() if norm > 100]
            errors.append(f"   Affected layers: {', '.join(exploding_layers[:5])}")
        elif max_norm > 10:
            warnings.append(
                f"⚠️  Large gradients: Max norm = {max_norm:.2e} (monitor closely)"
            )

        # Vanishing gradients
        if min_norm < 1e-7 and min_norm > 0:
            errors.append(
                f"❄️  VANISHING GRADIENTS: Min norm = {min_norm:.2e} (threshold: 1e-7)"
            )
            vanishing_layers = [
                name for name, norm in norms.items() if norm < 1e-7 and norm > 0
            ]
            errors.append(f"   Affected layers: {', '.join(vanishing_layers[:5])}")
        elif min_norm < 1e-5 and min_norm > 0:
            warnings.append(
                f"⚠️  Small gradients: Min norm = {min_norm:.2e} (monitor closely)"
            )
        ...
        if max_norm > 0 and min_norm > 0:
            ...
            if ratio > 1000:
                ...
                info.append(
                    "   → Indicates potential gradient flow issues across depth"
                )
    ...
    for name, stats in grad_stats.items():
        ...
        if zero_percent > 50:
            warnings.append(
                f"💀 {name}: {zero_percent:.1f}% zero gradients (dead neurons)"
            )
...
def main():
    ...
    parser.add_argument(
        "--checkpoint", type=str, help="Path to model checkpoint (.pt, .pth)"
    )
    parser.add_argument(
        "--data",
        type=str,
    ...
    parser.add_argument("--log-dir", type=str, help="Directory to save diagnostic logs")
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    if not args.checkpoint:
        ...
        print("\nExample usage:")
        print(
            "  python diagnose_gradients.py --checkpoint model.pt --data sample_data.pt"
        )
        sys.exit(1)
