...
def analyze_activation_statistics(
    ...
    if activation_type.lower() == "sigmoid":
        ...
        stats["saturation_percent"] = (
            100 * (saturated_low + saturated_high) / stats["total_activations"]
        )
    ...
    for layer_name, stats in activation_stats.items():
        ...
        if "dead_neuron_percent" in stats:
            if stats["dead_neuron_percent"] > 50:
                errors.append(
                    f"💀 {layer_name}: {stats['dead_neuron_percent']:.1f}% dead neurons (ReLU)"
                )
                errors.append(
                ...
                warnings.append(
                    f"⚠️  {layer_name}: {stats['dead_neuron_percent']:.1f}% dead neurons"
                )
        ...
        if "saturation_percent" in stats:
            if stats["saturation_percent"] > 80:
                errors.append(
                    f"📉 {layer_name}: {stats['saturation_percent']:.1f}% saturated activations"
                )
                errors.append(
                ...
                warnings.append(
                    f"⚠️  {layer_name}: {stats['saturation_percent']:.1f}% saturated"
                )

        # Very small activations (potential vanishing)
        if stats["max"] < 1e-5:
            warnings.append(
                f"❄️  {layer_name}: Max activation = {stats['max']:.2e} (very small)"
            )

        # Very large activations (potential exploding)
        if stats["max"] > 1e5:
            warnings.append(
                f"🔥 {layer_name}: Max activation = {stats['max']:.2e} (very large)"
            )
        ...
        if stats["mean"] != 0:
            ...
            if abs(skew) > 2:
                info.append(
                    f"📊 {layer_name}: Highly skewed distribution (skew={skew:.2f})"
                )
...
def print_activation_report(
    ...
    for layer_name, stats in activation_stats.items():
        ...
        print(
            f"{layer_display:<45} {stats['mean']:>12.4f} {stats['std']:>12.4f} "
    ...
    if verbose:
        ...
        for layer_name, stats in activation_stats.items():
            ...
            print(
                f"  Min: {stats['min']:.6f}, Median: {stats['median']:.6f}, Max: {stats['max']:.6f}"
            )
            print(
                f"  Percentiles: 1%={stats['p01']:.4f}, 10%={stats['p10']:.4f}, "
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, required=True, help="Path to model checkpoint (.pt, .pth)"
    )
    parser.add_argument("--data", type=str, help="Path to sample data (.pt file)")
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    print(
        "  5. Analyze: stats = {name: analyze_activation_statistics(hook.activations, name)"
    )
