...
def generate_speedup_chart(output_dir):
    ...
    ax.set_title(
        "JAX Transformation Performance Speedups",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
...
def generate_memory_optimization_chart(output_dir):
    ...
    compute_overhead = [
        ...
        100,
        100,
    ]  # Percentage of baseline (lower is better for time)
    ...
    ax.set_title(
        "Memory Optimization Techniques - Memory vs Compute Trade-offs",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
    ...
    ax.axhline(
        y=100, color="red", linestyle="--", alpha=0.5, linewidth=2, label="Baseline"
    )
...
def generate_batch_size_scaling(output_dir):
    ...
    ax1.set_title(
        "Batch Size Scaling - Throughput", fontsize=16, fontweight="bold", pad=20
    )
    ...
    ax2.set_title(
        "Multi-GPU Scaling Efficiency", fontsize=16, fontweight="bold", pad=20
    )
...
def generate_optimizer_comparison(output_dir):
    ...
    ax.bar(
        x,
        [10 - m for m in memory_usage],
        width,
        label="Memory Efficiency (10-usage)",
        color="#2ca02c",
        alpha=0.8,
        edgecolor="black",
        linewidth=1.5,
    ...
    ax.bar(
        x + width,
        [a - 90 for a in final_accuracy],
        width,
        label="Final Accuracy (%-90)",
        color="#ff7f0e",
        alpha=0.8,
        edgecolor="black",
        linewidth=1.5,
    ...
    ax.set_ylabel(
        "Relative Performance (arbitrary units)", fontsize=14, fontweight="bold"
    )
    ax.set_title(
        "Optimizer Comparison - Speed, Memory, and Accuracy",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
...
def generate_training_progress(output_dir):
    ...
    lr = np.where(
        ...
        0.001
        * (1 + np.cos(np.pi * (steps - warmup_steps) / (10000 - warmup_steps)))
        / 2,
    )
    ...
    ax1.set_title(
        "Training Progress - Loss Curves", fontsize=16, fontweight="bold", pad=20
    )
