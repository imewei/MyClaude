...
def example_production_training():
    ...
    print(
        f"Model created with {sum(p.size for p in jax.tree_leaves(nnx.state(model)))} parameters"
    )
    ...
    optimizer = nnx.Optimizer(
        model, optax.adamw(learning_rate=schedule, weight_decay=config["weight_decay"])
    )
