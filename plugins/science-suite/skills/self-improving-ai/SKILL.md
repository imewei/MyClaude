...
description: Build self-improving AI systems via programmatic prompt optimization (DSPy), inference-time scaling (best-of-N, tree-of-thoughts, MCTS), self-refinement loops (Self-Refine, STaR, Reflexion), and Constitutional AI / RLAIF training. Also covers autonomous research loops with multi-agent orchestration (AutoGen, CrewAI, LangGraph, smolagents) and evolutionary prompt-program search. Use when an LLM application needs to improve its own outputs without manual prompt-tuning, when a verifier is available and inference budget can grow, when preference data should be AI-generated rather than human-labeled, or when building multi-step autonomous research / coding / analysis agents. Use proactively when the user mentions DSPy, MIPRO, BootstrapFewShot, RLAIF, Constitutional AI, test-time compute, Self-Refine, STaR, Reflexion, AlphaEvolve, autonomous research, or self-improving AI.
---
