...
Everything above this line is the unmodified upstream command (`ecc:code-review`). The following is dev-suite-specific wiring, added without altering the copied text above.

- **Phase 2 (Local Review Mode)** and **Phase 3 (PR Review Mode)** — delegate to `quality-specialist` (loads `dev-suite:code-review`'s 6-step process: Context → Architecture → Logic → Security & Perf → Tests → Feedback) instead of reviewing inline. Same checklists and severity tiers above still apply as `quality-specialist`'s scope.
- **Phase 4 (VALIDATE)** — for Python projects, prefer `uv run pytest` and `uv run ruff check .` over bare `pytest` per this repo's tooling.
- See also `/review-pr` — multi-agent fan-out (`code-reviewer`, `comment-analyzer`, `pr-test-analyzer`, `silent-failure-hunter`, `type-design-analyzer`, `code-simplifier`), PR-only, report-only, no GitHub publish. This command is single-pass, covers local diffs too, and can publish to GitHub.
- `/git-branch finish` invokes this command as its review gate for the no-PR/MR direct-merge path (and `/review-pr` for the PR/MR path) before committing, pushing, or merging.
- **Naming note**: `/code-review` (this slash command) and `code-review` (the skill at `skills/code-review/SKILL.md`, `disable-model-invocation: true`) share a name but live in different Claude Code namespaces — not a collision. `quality-specialist` invokes the skill internally; you invoke the command directly.
