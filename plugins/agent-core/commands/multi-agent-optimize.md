...
allowed-tools: Read, Grep, Glob, Bash(git:*), Bash(uv:*)
...
agents:
  ...
  conditional:
    ...
    - agent: jax-pro
      trigger: pattern "jax|flax|@jit|@vmap|@pmap"
    - agent: neural-architecture-engineer
      trigger: pattern "torch|pytorch|tensorflow|keras"
    - agent: code-quality
      trigger: argument "--focus=quality"
  orchestrated: true
  execution: parallel
---
...
4. **Invoke agents**: Available agents analyze in parallel
