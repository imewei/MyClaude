"""Base agent interface for scientific computing agents.

This module defines the abstract base class and data models that all
scientific computing agents must implement. It extends the materials-science
agent framework for computational method specialization.
"""
...
class BaseAgent(ABC):
    """Abstract base class for all scientific computing agents.
    ...
    def _compute_cache_key(self, input_data: Dict[str, Any]) -> str:
        ...
        """
        # Create a JSON-serializable copy, replacing non-serializable objects
        serializable_data = {}
        for key, value in input_data.items():
            if callable(value):
                # For functions, use their code or id as proxy
                try:
                    serializable_data[key] = f"<function:{value.__name__}>"
                except AttributeError:
                    serializable_data[key] = f"<callable:{id(value)}>"
...
class ComputationalAgent(BaseAgent):
    ...
    Examples: Numerical solvers, optimization, ML models
    """
