...
This agent has been split into specialized agents for better organization:

- Use XRayScatteringAgent for: SAXS, WAXS, GISAXS, RSoXS, XPCS, time-resolved scattering
- Use XRaySpectroscopyAgent for: XAS, XANES, EXAFS

This unified agent is maintained for backward compatibility but will be removed in v3.0.0.
All new code should use the specialized agents.

Rationale: Scattering (measuring structure) and spectroscopy (measuring electronic states)
are fundamentally different techniques that deserve separate, focused implementations.
"""
...
class XRayAgent(ExperimentalAgent):
    ...
    ⚠️ DEPRECATED - Use XRayScatteringAgent or XRaySpectroscopyAgent instead ⚠️

    This unified agent has been deprecated in favor of specialized agents:
    - XRayScatteringAgent: SAXS, WAXS, GISAXS, RSoXS, XPCS, time-resolved
    - XRaySpectroscopyAgent: XAS, XANES, EXAFS

    Maintained for backward compatibility only. Will be removed in v3.0.0.
    """

    VERSION = "2.0.0"  # Deprecation version

    # ⚠️ SUPPORTED_TECHNIQUES REMOVED to prevent duplication ⚠️
    # This agent is DEPRECATED. All techniques have been moved to specialized agents.
    # Use XRayScatteringAgent or XRaySpectroscopyAgent instead.
    SUPPORTED_TECHNIQUES = []  # Empty - use specialized agents

    # Deprecation mapping to new specialized agents
    TECHNIQUE_MIGRATION_MAP = {
        'saxs': 'XRayScatteringAgent',
        'waxs': 'XRayScatteringAgent',
        'gisaxs': 'XRayScatteringAgent',
        'rsoxs': 'XRayScatteringAgent',
        'xpcs': 'XRayScatteringAgent',
        'time_resolved': 'XRayScatteringAgent',
        'xas': 'XRaySpectroscopyAgent',
        'xanes': 'XRaySpectroscopyAgent',
        'exafs': 'XRaySpectroscopyAgent',
    }
    ...
    def execute(self, input_data: Dict[str, Any]) -> AgentResult:
        ...
        ⚠️ DEPRECATED - This method issues deprecation warnings.
        ...
        Returns:
            AgentResult with X-ray analysis results and deprecation warnings
        """
        ...
        deprecation_warnings = [
            f"⚠️ XRayAgent is deprecated. Please migrate to {self.TECHNIQUE_MIGRATION_MAP.get(technique, 'specialized agent')}.",
            f"Technique '{technique}' should use: {self.TECHNIQUE_MIGRATION_MAP.get(technique, 'unknown')}",
            "This unified agent will be removed in v3.0.0."
        ]
        ...
        try:
            ...
            return AgentResult(
                ...
                provenance=provenance,
                warnings=validation.warnings + deprecation_warnings
            )
