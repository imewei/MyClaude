"""Base agent interface for nonequilibrium physics agents.

This module defines the abstract base class and data models that all
nonequilibrium physics agents must implement.
"""
...
class BaseAgent(ABC):
    """Abstract base class for all nonequilibrium physics agents.
...
class SimulationAgent(BaseAgent):
    """Base class for simulation-based nonequilibrium agents.

    Examples: Transport, Active Matter, Driven Systems, Stochastic Dynamics
    """
...
class AnalysisAgent(BaseAgent):
    """Base class for analysis/theory agents.

    Examples: Fluctuation Theorems, Pattern Formation Analysis
    """

    def __init__(self, config: Optional[Dict[str, Any]] = None):
        super().__init__(config)
        self.analysis_mode = config.get('mode', 'standard') if config else 'standard'

    @abstractmethod
    def analyze_trajectory(self, trajectory_data: Any) -> Dict[str, Any]:
        """Analyze simulation trajectory or experimental time series.
    ...
    def compute_observables(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Compute physical observables from raw data.

        Args:
            data: Raw simulation or experimental data
...
class CoordinationAgent(BaseAgent):
    ...
    Examples: Nonequilibrium Master (multi-technique coordinator)
    """
    ...
    def design_workflow(self, goal: Dict[str, Any]) -> 'Workflow':
        ...
        Args:
            goal: Analysis goal specification
    ...
    def optimize_technique_selection(self, goal: Dict[str, Any]) -> List[str]:
        """Select optimal techniques for analysis goal.
...
class ConvergenceError(AgentError):
    """Simulation convergence error."""
    pass